...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try{
        ...
        if(checkForErrors($nazwa, $data, $kategoria, $adres, $miasto, $cenaNormalna, $cenaUlgowa, $opis, $allPaths)) {
            header('Location:dodaj_wydarzenie.php');
            exit;
        }
        else{
            $stmt = $pdo->prepare("
            INSERT INTO wydarzenia 
            (nazwa, data, kategoria, adres, miasto, id_organizacji, czyPlatne, czyUlgowe, cenaNormalna, cenaUlgowa, opis, zdjecia) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$nazwa, $data, $kategoria, $adres, $miasto, $uzytkownik['id_organizacji'], $czyPlatne, $czyUlgowe, $cenaNormalna, $cenaUlgowa, $opis, $allPaths]);
...
<html lang="pl">
    <head>
        ...
        <title>Dodaj wydarzenie - KulturOn</title>
    ...
    <form action="dodaj_wydarzenie.php" method="POST" enctype="multipart/form-data" class="input_container">
        <input type="text" id="nazwa_wydarzenia" name="nazwa_wydarzenia" placeholder="Nazwa wydarzenia" required>
        <input type="date" id="data_wydarzenia" name="data_wydarzenia" placeholder="Data wydarzenia" required>
        <input type="text" id="kategoria_wydarzenia" name="kategoria_wydarzenia" placeholder="Kategoria wydarzenia" required>
        <input type="text" id="adres_wydarzenia" name="adres_wydarzenia" placeholder="Adres wydarzenia" required>
        <input type="text" id="miasto_wydarzenia" name="miasto_wydarzenia" placeholder="Miasto wydarzenia" required>
        <div class="checkbox_container">
            <label for="czy_platne">Czy płatne?</label>
            <input type="checkbox" id="czy_platne" name="czy_platne">
        </div>
        <div class="checkbox_container">
            <label for="czy_ulgowe">Czy ulgowe?</label>
            <input type="checkbox" id="czy_ulgowe" name="czy_ulgowe">
        </div>
        <input type="text" id="cena_wydarzenia_normalna" name="cena_wydarzenia_normalna" placeholder="Cena wydarzenia (normalna)" required>
        <input type="text" id="cena_wydarzenia_ulgowa" name="cena_wydarzenia_ulgowa" placeholder="Cena wydarzenia (ulgowa)" required>
        <textarea id="opis_wydarzenia" name="opis_wydarzenia" placeholder="Opis wydarzenia (do 255 znaków)" required></textarea>
        <input type="file" id="zdjecia_wydarzenia" name="zdjecia_wydarzenia[]" accept="image/*" multiple required>
